...
import VisibleElementsNext from '../../../views/Settings/VisibleElementsNext.vue'
...
function createWrapper() {
	return mount(VisibleElementsNext, {
		global: {
			stubs: {
...
describe('VisibleElementsNext.vue', () => {
	beforeEach(() => {
		vi.clearAllMocks()
		loadStateMock.mockImplementation((_app: string, _key: string, fallback: unknown) => fallback)
	})

	it('defaults to disabled (legacy editor) when no state is provided', () => {
	...
	it('loads enabled state from initial state', () => {
		loadStateMock.mockImplementation((_app: string, key: string, fallback: unknown) => {
			if (key === 'visible_elements_next_enabled') return true
			return fallback
		})
	...
	it('saves enabled state when the switch is toggled', async () => {
		...
		expect(OCP.AppConfig.setValue).toHaveBeenCalledWith('libresign', 'visible_elements_next_enabled', '1')
	})
})
