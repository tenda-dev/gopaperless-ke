...
import SponsorshipSettings from '../../../views/Settings/SponsorshipSettings.vue'
...
function createWrapper() {
	return mount(SponsorshipSettings, {
		global: {
			stubs: {
...
describe('SponsorshipSettings.vue', () => {
	beforeEach(() => {
		vi.clearAllMocks()
	...
	it('loads enabled state from initial state', () => {
		loadStateMock.mockImplementation((_app: string, key: string, fallback: unknown) => {
			if (key === 'sponsorship_enabled') return true
			return fallback
		})
	...
	it('saves enabled state when the switch is toggled', async () => {
		...
		expect(OCP.AppConfig.setValue).toHaveBeenCalledWith('libresign', 'sponsorship_enabled', '1')
	})
})
